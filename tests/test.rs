#![allow(clippy::format_collect)]

use const_hex::Buffer;

#[test]
fn buffer_fmt() {
    let mut buffer = Buffer::<256, true>::new();
    buffer.format(&ALL);
    let s = format!("{buffer:?}");
    let pre = "Buffer(\"0x";
    let post = "\")";
    assert_eq!(&s[..pre.len()], pre);
    assert_eq!(&s[s.len() - post.len()..], post);
    assert_lower(&s[pre.len()..s.len() - post.len()]);
}

#[test]
fn buffer_prefix() {
    let mut buffer = Buffer::<256, true>::new();
    let s = buffer.format(&ALL);
    assert_eq!(&s[..2], "0x");
    assert_lower(&s[2..]);
}

#[test]
fn buffer_array_lower() {
    let mut buffer = Buffer::<256>::new();
    let s = buffer.format(&ALL);
    assert_lower(s);
}

#[test]
fn buffer_array_upper() {
    let mut buffer = Buffer::<256>::new();
    let s = buffer.format_upper(&ALL);
    assert_upper(s);
}

#[test]
fn buffer_slice_lower() {
    let mut buffer = Buffer::<256>::new();
    let s = buffer.format_slice(ALL);
    assert_lower(s);
}

#[test]
fn buffer_slice_upper() {
    let mut buffer = Buffer::<256>::new();
    let s = buffer.format_slice_upper(ALL);
    assert_upper(s);
}

#[test]
fn buffer_const_lower() {
    const BUFFER: Buffer<256> = Buffer::new().const_format(&ALL);
    assert_lower(BUFFER.as_str());
}

#[test]
fn buffer_const_upper() {
    const BUFFER: Buffer<256> = Buffer::new().const_format_upper(&ALL);
    assert_upper(BUFFER.as_str());
}

#[test]
#[cfg(feature = "alloc")]
fn encode_lower() {
    let encoded = const_hex::encode(ALL);
    assert_lower(&encoded);
}

#[test]
#[cfg(feature = "alloc")]
fn encode_upper() {
    let encoded = const_hex::encode_upper(ALL);
    assert_upper(&encoded);
}

#[test]
#[cfg(feature = "alloc")]
fn encode_lower_prefixed() {
    let encoded = const_hex::encode_prefixed(ALL);
    assert_eq!(&encoded[0..2], "0x");
    assert_lower(&encoded[2..]);
}

#[test]
#[cfg(feature = "alloc")]
fn encode_upper_prefixed() {
    let encoded = const_hex::encode_upper_prefixed(ALL);
    assert_eq!(&encoded[0..2], "0x");
    assert_upper(&encoded[2..]);
}

#[test]
#[cfg(feature = "alloc")]
fn decode_lower() {
    let decoded = const_hex::decode(ALL_LOWER).unwrap();
    assert_eq!(decoded, ALL);
    let decoded = const_hex::decode_to_array(ALL_LOWER).unwrap();
    assert_eq!(decoded, ALL);
}

#[test]
#[cfg(feature = "alloc")]
fn decode_upper() {
    let decoded = const_hex::decode(ALL_UPPER).unwrap();
    assert_eq!(decoded, ALL);
    let decoded = const_hex::decode_to_array(ALL_UPPER).unwrap();
    assert_eq!(decoded, ALL);
}

#[test]
#[cfg(feature = "alloc")]
fn roundtrips() {
    test_roundtrip("1234");
    test_roundtrip("00000000000011");
    test_roundtrip("0000000000000022");
    test_roundtrip("000000000000000033");
    test_roundtrip("00000000000000003344");
    test_roundtrip("05161049138038061049183398181016");
}

#[test]
#[cfg(feature = "alloc")]
fn roundtrip_long() {
    test_roundtrip("");
}

#[cfg(feature = "alloc")]
fn test_roundtrip(s: &str) {
    const_hex::check(s).expect(s);
    let decoded = const_hex::decode(s).expect(s);
    assert_eq!(decoded, hex::decode(s).expect(s));
    assert_eq!(const_hex::encode(&decoded), s);
}

#[test]
fn check() {
    assert_eq!(const_hex::check(""), Ok(()));
    assert!(const_hex::check_raw(""));

    assert_eq!(const_hex::check(ALL_LOWER), Ok(()));
    assert_eq!(const_hex::check(ALL_UPPER), Ok(()));
    assert!(const_hex::check_raw(ALL_LOWER));
    assert!(const_hex::check_raw(ALL_UPPER));

    let error_cases = [
        ("ag", 1, 'g'),
        ("0xbz", 3, 'z'),
        ("0x12340000000n", 13, 'n'),
    ];
    for (s, index, c) in error_cases {
        assert_eq!(s[index..].chars().next(), Some(c), "{s:?}");
        assert_eq!(
            const_hex::check(s),
            Err(const_hex::FromHexError::InvalidHexCharacter { c, index })
        );
    }
}

#[test]
#[cfg(all(feature = "serde", feature = "alloc"))]
fn serde() {
    #[derive(serde::Serialize, serde::Deserialize)]
    struct All {
        #[serde(with = "const_hex")]
        x: Vec<u8>,
    }

    let all = All { x: ALL.to_vec() };
    let encoded = serde_json::to_string(&all).unwrap();
    assert_eq!(encoded, format!(r#"{{"x":"0x{ALL_LOWER}"}}"#));
    let decoded: All = serde_json::from_str(&encoded).unwrap();
    assert_eq!(decoded.x, ALL);
}

const ALL: [u8; 256] = [
    ,
];

const ALL_LOWER: &str = "";
const ALL_UPPER: &str = "";

// SIMD boundary tests - test at exact chunk sizes and off-by-one
#[test]
fn check_simd_boundaries() {
    // Test valid hex at SIMD chunk boundaries (SSE2=16, AVX2=32, and common sizes)
    for size in [1, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129, 255, 256, 257] {
        let valid_lower: String = "a".repeat(size);
        let valid_upper: String = "F".repeat(size);
        let valid_mixed: String = "0aF".chars().cycle().take(size).collect();

        assert!(const_hex::check_raw(&valid_lower), "failed at size {size}");
        assert!(const_hex::check_raw(&valid_upper), "failed at size {size}");
        assert!(const_hex::check_raw(&valid_mixed), "failed at size {size}");

        // Test invalid char at various positions
        for pos in [0, size / 2, size.saturating_sub(1)] {
            if pos < size {
                let mut invalid = valid_lower.clone().into_bytes();
                invalid[pos] = b'g'; // invalid hex char
                assert!(
                    !const_hex::check_raw(&invalid),
                    "should fail at size {size}, pos {pos}"
                );
            }
        }
    }
}

#[test]
#[cfg(feature = "alloc")]
fn encode_simd_boundaries() {
    // Test encode at SIMD chunk boundaries
    for size in [1, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129] {
        let input: Vec<u8> = (0..size).map(|i| i as u8).collect();
        let encoded = const_hex::encode(&input);
        assert_eq!(encoded.len(), size * 2, "encode failed at size {size}");

        // Verify against std format
        let expected: String = input.iter().map(|b| format!("{b:02x}")).collect();
        assert_eq!(encoded, expected, "encode mismatch at size {size}");
    }
}

#[test]
#[cfg(feature = "alloc")]
fn decode_simd_boundaries() {
    // Test decode at SIMD chunk boundaries (input is hex, so double the byte sizes)
    for size in [1, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129] {
        let bytes: Vec<u8> = (0..size).map(|i| i as u8).collect();
        let hex: String = bytes.iter().map(|b| format!("{b:02x}")).collect();

        let decoded = const_hex::decode(&hex).expect(&format!("decode failed at size {size}"));
        assert_eq!(decoded, bytes, "decode mismatch at size {size}");

        // Also test decode_to_slice
        let mut output = vec![0u8; size];
        const_hex::decode_to_slice(&hex, &mut output)
            .expect(&format!("decode_to_slice failed at size {size}"));
        assert_eq!(output, bytes, "decode_to_slice mismatch at size {size}");
    }
}

#[test]
fn encode_to_slice_simd_boundaries() {
    // Test encode_to_slice at SIMD chunk boundaries
    for size in [1, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129] {
        let input: Vec<u8> = (0..size).map(|i| i as u8).collect();
        let mut output = vec![0u8; size * 2];

        const_hex::encode_to_slice(&input, &mut output)
            .expect(&format!("encode_to_slice failed at size {size}"));

        let expected: String = input.iter().map(|b| format!("{b:02x}")).collect();
        assert_eq!(output, expected.as_bytes(), "mismatch at size {size}");
    }
}

#[test]
fn check_all_invalid_chars() {
    // Test that all non-hex characters are rejected
    for c in 0u8..=255 {
        let is_valid = matches!(c, b'0'..=b'9' | b'a'..=b'f' | b'A'..=b'F');
        let input = [c];
        assert_eq!(
            const_hex::check_raw(&input),
            is_valid,
            "char {c} (0x{c:02x}) should be {}",
            if is_valid { "valid" } else { "invalid" }
        );
    }
}

#[test]
#[cfg(feature = "alloc")]
fn roundtrip_simd_boundaries() {
    // Full roundtrip at SIMD boundaries
    for size in [1, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129, 255, 256] {
        let input: Vec<u8> = (0..=255u8).cycle().take(size).collect();
        let encoded = const_hex::encode(&input);
        let decoded = const_hex::decode(&encoded).unwrap();
        assert_eq!(input, decoded, "roundtrip failed at size {size}");
    }
}

// Edge case tests
#[test]
fn check_empty_input() {
    assert!(const_hex::check_raw(""));
    assert!(const_hex::check("").is_ok());
}

#[test]
#[cfg(feature = "alloc")]
fn encode_decode_empty() {
    assert_eq!(const_hex::encode(&[]), "");
    assert_eq!(const_hex::decode("").unwrap(), Vec::<u8>::new());
}

#[test]
fn check_boundary_chars() {
    // Characters just outside valid hex ranges
    // Before '0' (0x30): '/' is 0x2F
    assert!(!const_hex::check_raw("/"));
    // After '9' (0x39): ':' is 0x3A
    assert!(!const_hex::check_raw(":"));
    // Before 'A' (0x41): '@' is 0x40
    assert!(!const_hex::check_raw("@"));
    // After 'F' (0x46): 'G' is 0x47
    assert!(!const_hex::check_raw("G"));
    // Before 'a' (0x61): '`' is 0x60
    assert!(!const_hex::check_raw("`"));
    // After 'f' (0x66): 'g' is 0x67
    assert!(!const_hex::check_raw("g"));

    // Valid boundary characters
    assert!(const_hex::check_raw("0"));
    assert!(const_hex::check_raw("9"));
    assert!(const_hex::check_raw("A"));
    assert!(const_hex::check_raw("F"));
    assert!(const_hex::check_raw("a"));
    assert!(const_hex::check_raw("f"));
}

#[test]
fn check_high_bytes() {
    // Test bytes >= 0x80 (important for signed comparison trick)
    for c in 0x80u8..=0xFF {
        let input = [c];
        assert!(
            !const_hex::check_raw(&input),
            "byte 0x{c:02x} should be invalid"
        );
    }
}

#[test]
fn check_special_values() {
    // NUL byte
    assert!(!const_hex::check_raw(&[0x00]));
    // Space
    assert!(!const_hex::check_raw(" "));
    // Newline
    assert!(!const_hex::check_raw("\n"));
    // Tab
    assert!(!const_hex::check_raw("\t"));
    // DEL (0x7F)
    assert!(!const_hex::check_raw(&[0x7F]));
}

#[test]
#[cfg(feature = "alloc")]
fn decode_odd_length_error() {
    // Odd-length hex strings should fail
    assert!(const_hex::decode("a").is_err());
    assert!(const_hex::decode("abc").is_err());
    assert!(const_hex::decode("0x1").is_err());
}

#[test]
#[cfg(feature = "alloc")]
fn encode_all_byte_values() {
    // Ensure every byte value encodes correctly
    let input: Vec<u8> = (0..=255u8).collect();
    let encoded = const_hex::encode(&input);

    // Verify each byte
    for (i, byte) in input.iter().enumerate() {
        let hex_pair = &encoded[i * 2..i * 2 + 2];
        let expected = format!("{byte:02x}");
        assert_eq!(hex_pair, expected, "byte {byte} at index {i}");
    }
}

#[track_caller]
fn assert_lower(s: &str) {
    let expected = (0..=u8::MAX)
        .map(|i| format!("{i:02x}"))
        .collect::<String>();
    assert_eq!(ALL_LOWER, expected);
    assert_eq!(s, expected);
}

#[track_caller]
fn assert_upper(s: &str) {
    let expected = (0..=u8::MAX)
        .map(|i| format!("{i:02X}"))
        .collect::<String>();
    assert_eq!(ALL_UPPER, expected);
    assert_eq!(s, expected);
}
